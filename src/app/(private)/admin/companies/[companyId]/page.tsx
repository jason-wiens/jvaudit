...
export default function CompanyPage() {
  const { companies, changePrimaryContact, deleteEmployee } =
    useCompanyContext();
  const { addAlert } = useAlerts();
  const companyId = useStringParam("companyId");
  const [company, setCompany] = useState<Company | null>(null);
  const [updateEmployee, setUpdateEmployee] = useState<CompanyEmployee | null>(
    null
  );
  ...
  const handleChangingPrimary = async (employee: CompanyEmployee) => {
    await changePrimaryContact(companyId, employee);
  };

  const handleEditEmployee = async (employee: CompanyEmployee) => {
    setUpdateEmployee(employee);
  };

  const handleDeleteEmployee = async (employee: CompanyEmployee) => {
    console.log(`Deleting: ${employee.personalProfile.firstName}`);
  };
